 ...
 * @param fileMode  If true then forces files to be read from the filesystem
 *                  instead of via git; commitishs are ignored in this mode.
 ...
 * resolved and loaded.
 */
async function loadJSON( ref, fileMode = false, env = process.env, required = true ) {
    try {
        // Read the file.
        const source = await Source.parseSource( ref, fileMode );
        const json = await source.read();
        const data = JSON.parse( json );
        return resolve( data, source, fileMode, env );
    }
    catch( e ) {
 ...
 * @param fileMode  Flag indicating whether to only read files from the
 *                  file system.
 * @param env       Variables used to resolve path templates.
 * @returns The resolve property value.
 */
async function resolve( value, source, fileMode, env ) {
    switch( typeof value ) {
    case 'string':
        // Check for a link.
        if( value[0] == '@' ) {
            ...
            value = await loadJSON( _source, fileMode, env, required );
        }
        break;
    case 'object':
        // Resolve each item in the array or each property of object.
        if( Array.isArray( value ) ) {
            value = Promise.all( value.map( i => resolve( i, source, fileMode, env ) ) );
        }
        else for( let name in value ) {
            value[name] = await resolve( value[name], source, fileMode, env );
        }
        break;
    default:
        // Return value as-is.
